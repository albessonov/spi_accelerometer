...
void HAL_SPI_RxCpltCallback(SPI_HandleTypeDef *hspi)
{
  if(hspi == &hspi3)
  {
    ...
    { 
			 acceleration_32=form_acc(bufX[ctr0],X);
       timeX+=0.5;
       resp[0]=acceleration_32>>24;
       resp[1]=acceleration_32>>16;
       resp[2]=acceleration_32>>8;
       resp[3]=acceleration_32;
       ctr0+=1;
    }
		else if (memcmp(RXbuf,Request0x2cmd,4)==0)
    {  acceleration_32=form_acc(bufY[ctr2],Y);
       timeY+=0.5;
       resp[0]=acceleration_32>>24;
       resp[1]=acceleration_32>>16;
       resp[2]=acceleration_32>>8;
...
int main(void)
{
  ...
  HAL_TIM_Base_Start(&htim1);
	...
	{
		accX[i]=form_acc(bufX[i],X);
		accY[i]=form_acc(bufY[i],Y);
	}*/
  /* USER CODE END 2 */
  ...
  while (1)
  {
		...
		HAL_SPI_Receive_IT(&hspi3,RXbuf,4);//1000);
		
		while(!tx_ready){} 
			
		HAL_SPI_Transmit(&hspi3,resp,4, 1000);
		tx_ready = false;
